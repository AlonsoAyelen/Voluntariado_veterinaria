...
def nuevo_usuario(request):
	contexto = {'error':'' , 'nombre':'' , 'clave':'' , 'tipo':''}
...
def duenios_view(request):
	...
	if not request.user.is_authenticated():
		...
		if(len(request.POST.getlist('eliminar'))> 0):
			...
			duenios = Propietario.objects.filter()
			contexto = {'duenios':duenios}
		elif(len(request.POST.getlist('modificar'))> 0):
			duenio = Propietario.objects.filter(id = int(request.POST.getlist('modificar')[0]))
...
def caninos_view(request):
	if not request.user.is_authenticated():
		...
		contexto = {'caninos':caninos,'propietario':'' , 'nombre':'' ,'especie':'', 'sexo':'','raza':'','edad':'','motivo_de_consulta':'', 'uso_de_la_mascota':'','habitos':'' , 'contacto_con_basural':'' , 'caza':'','caza_roedores':'' ,	'observacion_roedores':'', 'vacunado_contra_leptospirosis':'','desparasitado':'' , 'eliminacion_de_excretas':'',	'habitos_alimenticios':'','signos_clinicos':'','piel_Linfonodos':'','digestivo':'','cardio_respiratorio':'','urogenital':'','musculoesqueleticonervioso':'','procedimiento_realizado':'','peso':'','actitud':'','mucosas':'','TLC':'','hidratacion':'','FC':'','pulso':'','FR':'','T':''}
		if(len(request.POST.getlist('eliminar'))> 0):
			...
			print request.POST.getlist('modificar')
			canino = Canino.objects.filter(id = int(request.POST.getlist('modificar')[0]))
			contexto['duenios'] = Propietario.objects.all() 
			contexto['nombre'] = canino[0].nombre
			contexto['especie'] = canino[0].especie
			contexto['sexo'] = canino[0].sexo
			contexto['raza'] = canino[0].raza
...
def usuarios_view(request):
	if not request.user.is_authenticated():
		...
		if(len(request.POST.getlist('eliminar'))> 0):
			...
			usuario = User.objects.filter(id = int(request.POST.getlist('modificar')[0]))
			contexto['nombre'] = usuario[0].username
			contexto['clave'] = 'existe clave' 
			contexto['tipo'] = usuario[0].is_staff
			contexto['estado'] = 'modificado'
...
def nuevo_canino_view(request):
	contexto = {'error':'','propietario':'' , 'nombre':'' ,'especie':'', 'sexo':'','raza':'','edad':'','motivo_de_consulta':'', 'uso_de_la_mascota':'1','habitos':'' , 'contacto_con_basural':'' , 'caza':'','caza_roedores':'' ,	'observacion_roedores':'', 'vacunado_contra_leptospirosis':'','desparasitado':'' , 'eliminacion_de_excretas':'',	'habitos_alimenticios':'','signos_clinicos':'','piel_Linfonodos':'','digestivo':'','cardio_respiratorio':'','urogenital':'','musculoesqueleticonervioso':'','procedimiento_realizado':''}
	if not request.user.is_authenticated():
		return redirect('login')
	else:
		if(request.method == 'POST'): 
